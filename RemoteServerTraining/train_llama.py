"""
Llama-3.1-8B Safety Fine-tuning — Wolpertinger Server Edition
3x NVIDIA GeForce RTX 3090 (24576MiB each)

Before running:
    export HF_TOKEN=hf_your_token_here   # required — Llama is a gated model

Run:
    python train_llama.py                                         # single-GPU (GPU 0)
    accelerate launch --multi_gpu --num_processes=3 train_llama.py  # 3-GPU DDP
"""
...
OUTPUT_DIR = os.path.join(HOME, "llama3.1-8b-safety")

MODEL_NAME     = "meta-llama/Llama-3.1-8B"
MAX_SEQ_LENGTH = 512

# LoRA — same as Qwen run
LORA_R       = 32
LORA_ALPHA   = 64
LORA_DROPOUT = 0.05
...
# Filter to English-only examples
...
# Llama-3.1 does not have a pad token by default — use eos token
...
# ──────────────────────────────────────────────────────────────────────────────
# Format dataset using Llama-3.1 native chat template
# ──────────────────────────────────────────────────────────────────────────────
...
# Llama-3.1-8B in bf16 = ~16GB — fits on RTX 3090 24GB
# For DDP (accelerate launch) use device_map=None
# ──────────────────────────────────────────────────────────────────────────────
from accelerate import PartialState
using_ddp = PartialState().num_processes > 1
device_map = None if using_ddp else "auto"

print(f"\nLoading model {MODEL_NAME}  (DDP={using_ddp})...")
...
training_args = SFTConfig(
    ...
    save_total_limit=12,        # 6 epochs × 2 evals per epoch = 12
